...
## Фаза 4: Формы и лиды
- [x] Задача 4.1: Сохранить Web3Forms как основной канал писем на `info@usholding.kz` (форма `#contactForm`, ключ из `access_key`)
- [x] Задача 4.2: Добавить дублирующую запись каждой заявки в таблицу `leads` (Supabase) через Route Handler — заявки не теряются, если письмо Web3Forms не дойдёт
...
- [x] Задача 6.1: Настроить Vitest для unit-тестов утилит и Route Handler'ов
- [x] Задача 6.2: Настроить Playwright для smoke e2e: главная страница, открытие карточки компании, вход через Clerk, отправка формы заявки, сохранение профиля
- [x] Задача 6.3: Настроить GitHub Actions: lint + typecheck + unit + e2e на каждый PR в `rebuild`/`main`, блокировать merge при провале
- [x] Задача 6.4: Включить Netlify Deploy Preview на PR для визуальной проверки перед мерджем

## Фаза 7: Админ-панель
- [x] Задача 7.1: Защищённый раздел `/admin`, доступ только роли `admin` (Clerk Organizations/Roles)
- [x] Задача 7.2: CRUD для `portfolio_projects` (создание/редактирование/удаление, загрузка фото в Supabase Storage)
- [x] Задача 7.3: CRUD для карточек `companies` (описание, услуги, теги — сейчас правится только через хардкод объекта `COS` в JS)
- [x] Задача 7.4: Просмотр и управление статусом заявок `leads` (Новая / В работе / Закрыта)

## Фаза 8: Запуск и пост-релиз
- [x] Задача 8.0 Починить Telegram-relay и добавить двустороннюю связь
- [x] Задача 8.1: Подготовить конфиги для деплоя на Hetzner (ecosystem.config.cjs, deploy.sh, nginx/usholding.conf, next.config.ts standalone, убрать @netlify/plugin-nextjs)
- [ ] Задача 8.2: Установить nginx + PM2 на Hetzner, настроить .env.local, запустить `deploy.sh` (требует sudo)
- [ ] Задача 8.3: Настроить Cloudflare DNS: A-запись `usholding.kz` → 62.238.25.150, Proxy mode ON (SSL/CDN), проверить `clerk.usholding.kz`
- [ ] Задача 8.4: Финальная проверка `usholding.kz` на Hetzner — все секции, 17 модалок, формы, авторизация
- [ ] Задача 8.5: Решить судьбу ветки `cloudflare/workers-autoconfig` — удалить либо задокументировать
